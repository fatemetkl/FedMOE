...
def visualize_relative_server_squared_errors(
    ...
    for i in range(game_server_predictions.shape[1]):
        relative_error = game_squared_error[:, i] / non_game_squared_error[:, i]
        plt.scatter(
            time_axis,
            relative_error.detach().numpy(),
            label=f"$\\frac{{(\\hat{{Y}}^g_{i+1} - y_{i+1})^2}}{{(\\hat{{Y}}^n_{i+1} - y_{i+1})^2}}$",
        )
    ...
    plt.title(f"Ratio Squared Errors with and Without Nash Game", fontdict=title_font)

    plt.legend(prop={"family": "helvetica", "weight": "bold", "size": 22}, labelspacing=0)
